/*
    HOW TO USE: AC_FSM ACA;
    ACA.construct_automaton(patterns)
    ACA.aho_corasick(text, patterns, matches)
    matches[i] is the patterns that appear on the i-th position of the text
*/
struct AC_FSM
{
    #define ALPHABET_SIZE 26
    const int OFFSET = 'a'; ///or A according to problem
    struct Node
    {
    ...
    void construct_automaton(vector<string> &words) {
        for (int w = 0, n = 0; w < words.size(); ++w, n = 0) {
            for (int i = 0; i < words[w].size(); ++i) {
                if (a[n].child[words[w][i] - OFFSET] == -1) {
                    a[n].child[words[w][i] - OFFSET] = a.size();
                    a.push_back(Node());
                }
                n = a[n].child[words[w][i] - OFFSET];
            }
    ...
    {
        matches.assign(sentence.size(), vector<int>());
        int state = 0, ss = 0;
        for (int i = 0; i< sentence.length(); ++i, ss = state)
        {
            while (a[ss].child[sentence[i] - OFFSET] == -1)
                ss = a[ss].failure;
            state = a[state].child[sentence[i] - OFFSET] = a[ss].child[sentence[i] - OFFSET];
