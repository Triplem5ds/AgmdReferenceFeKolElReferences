...
def get_sections(path):
    sections = []
    section_name = None
    str = os.path.join(path[0:len(path)],'contents.txt')
    with open(os.path.join(path[0:len(path)],'contents.txt'), 'r') as f:
        for line in f:
...
if __name__ == "__main__":
    ...
    with open('contents_'+basepath+'.tex', 'w+') as f:
        f.write(tex)
    latexmk_options = ["pdflatex", "notebook_"+basepath+".tex"]
